...
fim = im2single(rgbim) ; % converts the grayscale, RGB, or binary image I to single, rescaling or offsetting the data as necessary
fim = single(featurefun(fim)) ;

rgbim = permute(rgbim, [3 2 1]) ; % rearranges the dimensions of an array in the order specified by the vector dimorder
...
if p.steps == 1
    ...
    % perform synergistic segmentation, default is true
    if p.synergistic
