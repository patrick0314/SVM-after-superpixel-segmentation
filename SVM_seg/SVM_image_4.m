function [segments,tmp,Hist,HistDiff,area,Edge,DTex,meanCC,meanLGTex,meanSaliency,adj,break_while] = SVM_image_4(segments, predicted, stage, Hist, HistDiff, area, Edge, DTex, LabIm, meanCC, textureImg, meanLGTex, SaliencyMap, meanSaliency, adj, LongContourMap25, LongContourMap80, edge_map, contact_rate, dSV)
    maxL = size(adj, 1) ;
    ...
    if stage == 1
        ...
        for order = 1:10
            ...
            if (i > j) && contact_rate(i, j) >= 0.125 && dSV(i, j) <= 0.1
                region_i = (segments==i) ;
                region_j = (segments==j) ;
