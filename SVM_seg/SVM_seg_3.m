function [label, features, contact_rate, dSV] = SVM_seg_3(segments,idx,BSDS_INFO,HistDiff,area,Edge,DTex,meanCC,adj,meanSaliency)
    ...
    for i = 1:maxL
        if find(segments==i)
            ...
            SizeNeighbor = length(neighbor) ;
            for j = 1:SizeNeighbor
                Edge_f1(i, neighbor(j)) = minArea(i, neighbor(j)) / W * Edge(i, neighbor(j)).Strength ;
                Edge_f2(i, neighbor(j)) = minArea(i, neighbor(j)) / W * Edge(i, neighbor(j)).Rate25 ;
                Edge_f3(i, neighbor(j)) = Edge(i, neighbor(j)).Rate80 ;
            end
        end
    end
    Edge_f1 = Edge_f1(:) ;
    ...
    features = cat(2, HistDiff_f, Edge_f1, Edge_f2, Edge_f3, DTex_f, de00_f, dSV_f, minArea_f) ;
