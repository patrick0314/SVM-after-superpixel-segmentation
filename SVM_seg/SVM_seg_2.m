function [label, features, contact_rate, dSV] = SVM_seg_2(segments,idx,BSDS_INFO,HistDiff,area,Edge,DTex,meanCC,adj,meanSaliency)
    ...
    HistDiff_f = replace(HistDiff(:)) ;
    % (Edge(L1,neighbor(k)).Strength*5+Edge(L1,neighbor(k)).Rate25*0.5)*min(minArea/W,2)
    ...
    for i = 1:maxL
        if find(segments==i)
            ...
            SizeNeighbor = length(neighbor) ;
            for j = 1:SizeNeighbor
                Edge_f1(i, neighbor(j)) = Edge(neighbor(j), i).Strength .* 5 .* min(minArea(i, neighbor(j))/W, 2) ;
                Edge_f2(i, neighbor(j)) = Edge(i, neighbor(j)).Rate25 .* 0.5 .* min(minArea(i, neighbor(j))/W, 2) ;
            end
        end
    end
    Edge_f1 = Edge_f1(:) ;
