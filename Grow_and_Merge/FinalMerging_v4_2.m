function [segments,score,Hist,HistDiff,area,Edge,DTex,meanCC,meanLGTex,meanSaliency,adj,Rnum]=FinalMerging_v4_2(segments,score, Hist, HistDiff,area,Edge,DTex,LabIm,meanCC,textureImg,meanLGTex,SaliencyMap,meanSaliency,adj,LongContourMap25,LongContourMap80,edge_map,Rnum,Nseg)
    ...
    if conti
        for order=1:length(Loc)
            ...
            contact_rate=length(BndInd)*0.5/min( L1_border_length, L2_border_length);
            dSV=abs(meanSaliency(L1)-meanSaliency(L2));
            if  contact_rate>=0.06 && dSV<=0.4 && dSV>0.125
                idxL2= (segments==L2); len2=sum(idxL2(:));
                idxL1= (segments==L1); len1=sum(idxL1(:));
                segments(idxL2)=L1; % merge segment L2 into L1
                Rnum=Rnum-1;
